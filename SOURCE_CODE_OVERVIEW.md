...
| Thống kê | Số lượng |
|----------|----------|
| File Java (`src/main/java`) | **~100** |
| Servlet đã triển khai | **~30** |
| DAO | **14** |
| Entity | **12** |
| DTO | **6** |
| Filter | **3** |
| Util | **18** |
| JSP view | **~31** (+ 5 `.gitkeep`) |
| CSS | **11** |
| JS | **7** |
| Script SQL | `create_database.sql` |
| Screen Design (mockup HTML) | **3 màn** (Cinema Auditoriums, Seat Layout, Showtime Management) |
...
| `MovieDAO` | `Movies`, `MovieGenres` | Public listing, featured, search, early showtimes; manager CRUD; `getSchedulableMovies()`; `hasActiveShowtimes()` |
...
| `ShowtimeDAO` | `Showtimes` | Counter: suất theo phim; Manager: `getAllForManager`, create, update, delete, `isOverlapping`, `countBookingsByShowtimeId` |
...
5. **~12/26 bảng** chưa có lớp DAO tương ứng — chỉ schema + seed. Đã cải thiện so với trước nhờ thêm `VatRuleDAO`, `SystemConfigDAO`, `SystemConfigLogDAO`, `SeatTypeDAO` full CRUD, `ShowtimeDAO` manager CRUD.
