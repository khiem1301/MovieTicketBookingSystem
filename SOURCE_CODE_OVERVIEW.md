...
Tài liệu nghiệp vụ đầy đủ (26 bảng, 50 FR): [`project_summary_final.md`](project_summary_final.md)  
...
├── Screen Design/                   # Mockup HTML + DESIGN.md (chưa wire trực tiếp vào build)
│   ├── Cinema Auditoriums/          # UI danh sách phòng chiếu
│   └── Seat Layout/                 # UI editor layout ghế
├── Database/
...
| `/manager/rooms` | `ManageCinemaRoomServlet` | GET | Danh sách phòng chiếu (FR-26 UI) |
| `/manager/rooms/detail?id={uuid}` | `ManageCinemaRoomServlet` | GET | Chi tiết phòng + editor layout ghế (frontend) |

> Chi tiết endpoint, UI và hạn chế → [`MANAGER_MODULE_DETAIL.md`](MANAGER_MODULE_DETAIL.md)  
...
#### `ManageCinemaRoomServlet` — `/manager/rooms`, `/manager/rooms/detail`
- **GET list:** đọc `CinemaRooms` qua `CinemaRoomDAO`, hiển thị grid + panel preview (`cinema-room-list.jsp`)
- **GET detail:** load phòng theo `id`, loại ghế từ `SeatTypeDAO`, forward `cinema-room-detail.jsp`
- **Layout ghế:** editor tương tác thuần frontend (`manager-seat-layout.js`) — lưu bản nháp `localStorage`, **chưa** ghi `Seats` vào DB
- Metadata hiển thị (chip IMAX, suất hiện tại, occupancy) trên list: derive tạm trong servlet — chờ backend đầy đủ
- Chưa có POST: thêm/sửa phòng, toggle trạng thái, lưu layout ghế
...
| `CinemaRoomDAO` | `CinemaRooms`, `Seats` (read) | getAll, getById, countActiveSeats, countAccessibleSeats |
| `SeatTypeDAO` | `SeatTypes` | getAll — dùng sidebar editor layout ghế |
...
| `SeatType` | `SeatTypes` | |
| `CinemaRoom` | `CinemaRooms` | Có `CinemaRoomDAO` — manager list/detail; chưa CRUD |
| `Booking` | `Bookings` | |
...
| File | Mô tả |
|------|-------|
| `common/header.jsp` | Logo, search, nav, dropdown theo role; hỗ trợ `extraCss` + `extraCss2` |
...
| `cinema-room-list.jsp` | Danh sách phòng chiếu — design *Cinema Auditoriums* |
| `cinema-room-detail.jsp` | Chi tiết phòng + editor layout ghế — design *Seat Layout* |

**Luồng UI phòng chiếu (FR-26 — giai đoạn 1):**

```
/manager/rooms  →  card / panel preview  →  nút「Chi tiết」/「Xem chi tiết」
       →  /manager/rooms/detail?id=...  →  editor ghế (chỉ ở trang này)
```

Menu dropdown MANAGER (`header.jsp`): Quản lý phim · Quản lý thể loại · **Quản lý phòng chiếu** (chỉ role MANAGER).
...
| File | Dùng bởi |
|------|----------|
| `css/main.css` | Layout chung, homepage, movies, `.mgr-*` (phim/thể loại) |
| `css/manager-auditoriums.css` | UI danh sách phòng chiếu (`.aud-*`, glass panel) |
...
| `js/manager-auditoriums.js` | Lọc phòng, chọn card, cập nhật panel preview |
| `js/manager-seat-layout.js` | Editor ghế client-side (select / add / gap; draft localStorage) |
