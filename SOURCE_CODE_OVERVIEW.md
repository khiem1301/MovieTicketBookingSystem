# ÉPCINE — Tổng hợp Source Code (đến 09/06/2026)
...
└─────────────────────────────────────────────────────────┘
   ...
   ▼
┌─────────────────────────────────────────────────────────┐
│  SQL Server — MovieTicketDB (27 bảng)                   │
└─────────────────────────────────────────────────────────┘
```
...
├── Database/
│   ├── README.md                    # Hướng dẫn DB & migration
│   ├── create_database.sql          # Schema + seed (27 bảng)
│   └── migrations/                  # Cập nhật incremental
...
| `/admin/config` | `SystemConfigListServlet` | Cấu hình loyalty + lịch sử chỉnh sửa |
| `/admin/config/update` | `SystemConfigUpdateServlet` | POST: lưu cấu hình loyalty |
| `/admin/vat` | `VatRuleListServlet` | Quản lý thuế VAT |
| `/admin/vat/create` | `VatRuleCreateServlet` | POST: tạo rule VAT mới |
| `/admin/vat/update` | `VatRuleUpdateServlet` | POST: sửa rule đã lên lịch |
| `/admin/vat/cancel` | `VatRuleCancelServlet` | POST: hủy rule đã lên lịch |
...
- Tính VAT từ `VatRuleDAO.findCurrentActiveRate()` (qua `BookingDAO`)
...
| `SystemConfigDAO` | `SystemConfig` | findByKeys, updateValue |
| `SystemConfigLogDAO` | `SystemConfigLog` | insert, findLoyaltyHistory |
| `VatRuleDAO` | `VatRules` | findEffectiveNow, findScheduledList, createAndActivate, updateScheduled, cancelScheduled |

**Bảng có schema nhưng chưa có DAO riêng:** `Payments`, `Promotions`, `Tickets`, `LoyaltyPointsLog`, `ShowtimeIncidents`, `ChatbotConversations`, `ChatbotMessages`, `PricingRules`, `SeatHolds`, `MovieReviews`, `CinemaInfo`
...
| `SystemConfigLog` | `SystemConfigLog` | Lịch sử loyalty |
| `VatRule` | `VatRules` | |
...
**Script khởi tạo:** `Database/create_database.sql` (~1010 dòng) — chạy một lần trên SSMS/Azure Data Studio.

**Migration (DB đã có):** `Database/migrations/` — xem [`Database/README.md`](Database/README.md).
...
| Admin — Cấu hình loyalty | ✅ Hoàn thành | 4 key + lịch sử `SystemConfigLog` |
| Admin — Cấu hình VAT | ✅ Hoàn thành | `/admin/vat` — VatRules |
...
| System Config UI (loyalty) | ✅ Hoàn thành | `/admin/config` + lịch sử chỉnh sửa |
...
*Tài liệu được tổng hợp từ source code thực tế trong repo, cập nhật 09/06/2026.*
