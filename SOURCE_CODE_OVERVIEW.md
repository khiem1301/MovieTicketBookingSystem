# ÉPCINE — Tổng hợp Source Code (đến 14/06/2026)
...
- **POST save-layout:** parse JSON layout qua `SeatLayoutJsonUtil.parseSeats()` → persist vào bảng `Seats`
...
| `MovieDAO` | `Movies`, `MovieGenres` | Public listing, featured, search, early showtimes; manager CRUD (create/update/soft delete); `getAllGenres()`, `hasActiveShowtimes()` |
...
| `CinemaRoomDAO` | `CinemaRooms` | getAll, getById, create, updateName, updateStatus; existsByName*; countUpcomingShowtimes, countActiveSeats, countAccessibleSeats |
...
**Bảng có schema nhưng chưa có DAO:** `Payments`, `Promotions`, `Tickets`, `LoyaltyPointsLog`, `ShowtimeIncidents`, `ChatbotConversations`, `ChatbotMessages`, `PricingRules`, `SeatHolds`, `MovieReviews`, `CinemaInfo`, `BookingPromotions`
...
| `SeatLayoutJsonUtil` | Serialize (`buildLayoutJson`) / deserialize (`parseSeats`) layout ghế JSON ↔ `Seat` entities |
...
| Manager — Phòng chiếu | 🟡 Đang triển khai | Tạo, rename, toggle status ✅; save layout ✅; xóa phòng ❌ |
...
5. **~12/26 bảng** chưa có lớp DAO tương ứng — chỉ schema + seed. Đã cải thiện so với trước nhờ thêm `VatRuleDAO`, `SystemConfigDAO`, `SystemConfigLogDAO`, `SeatTypeDAO` full CRUD.
6. **Xóa phòng chiếu** chưa có — chỉ tạo, rename, toggle status.
7. **Không có test tự động** — JUnit dependency có nhưng chưa viết test.

---

*Tài liệu được tổng hợp từ source code thực tế trong repo, cập nhật 14/06/2026.*
