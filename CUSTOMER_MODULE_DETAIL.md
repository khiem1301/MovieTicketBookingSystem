...
├── ProfileServlet.java            # /profile — FR-04/05 (mọi role đã login)
├── ChangePasswordServlet.java     # /profile/password — FR-04/05
├── ProfileSecurityVerifyServlet.java    # /profile/security-verify
...
| Servlet | URL | Vai trò |
|---------|-----|---------|
| `ForgotPasswordServlet` | `/forgot-password` | FR-04: gửi link reset mật khẩu qua email |
| `ResetPasswordServlet` | `/reset-password` | FR-04: đặt lại mật khẩu qua token |
...
**Endpoint profile (mọi role đã login):**

| URL | Method | Mô tả |
|-----|--------|-------|
| `/profile` | GET, POST | Xem/sửa thông tin + upload avatar |
| `/profile/password` | POST | Đổi mật khẩu (sau xác minh bảo mật) |
| `/profile/security-verify` | GET, POST | Xác minh bằng mật khẩu hiện tại hoặc link email |
| `/profile/security-verify/confirm` | GET | Xác nhận token email → session verified 15 phút |
| `/forgot-password` | GET, POST | Quên mật khẩu (public) |
| `/reset-password` | GET, POST | Đặt lại mật khẩu qua token (public) |
...
**View profile & auth (dùng chung, không nằm trong `customer/`):**

```
src/main/webapp/WEB-INF/views/
├── common/
│   ├── profile.jsp                    # FR-05: trang tài khoản
│   ├── profile-security-verify.jsp    # FR-04/05: xác minh danh tính
│   └── profile-security.jspf          # Fragment form đổi mật khẩu
└── auth/
    ├── forgot-password.jsp            # FR-04: quên mật khẩu
    └── reset-password.jsp             # FR-04: đặt lại mật khẩu
```
...
| `js/profile.js` | Preview avatar, toggle form đổi mật khẩu |
...
|------|----------------------|
| `dal/UserDAO.java` | `getById`, **`updateProfile`**, `updatePasswordHash` — profile |
...
| `.pay-*` | Trang payment — breakdown VAT, countdown, QR VietQR, nút hủy | `Screen Design/Online payment/`; UI dùng thêm prefix legacy `.pay-momo-*` |
