...
**Trạng thái hiện tại (16/06/2026):** Đã triển khai **FR-11**, **FR-50** (UI giá động + seed), **FR-12**, **FR-13**, **FR-14**, **FR-22** (áp mã voucher trên `/payment`). Chưa có VNPay/MoMo (FR-16–18), loyalty, reviews.
...
> `ShowtimesServlet` ở `controller` (public). `CheckoutServlet` và `PaymentServlet` ở `controller.customer` (CUSTOMER-only qua `RoleFilter`).
...
└── customer/
    ...
    ├── PaymentServlet.java        # /payment — FR-14 / FR-22 / FR-16 stub (CUSTOMER)
    └── package-info.java
```
...
| `PaymentServlet` | `/payment` | Tóm tắt đơn + voucher (FR-22) + countdown (stub FR-16) |
...
| `/payment` | POST (khác) | Stub VNPay/MoMo (FR-16) |
...
| `css/customer-checkout.css` | Chọn ghế — `.ck-*`; payment — `.pay-*`, `.pay-promo-*` |
...
| `js/customer-payment.js` | Countdown hết hạn đơn; uppercase mã voucher khi gõ (FR-14/22) |
...
| `dal/BookingDAO.java` | FR-14/22: `createOnlineBooking`, **`applyPromotionToBooking`**, **`removePromotionFromBooking`**, **`cancelOnlinePendingBooking`**, `getDetailById`, `findActiveOnlinePendingBookingId` |
...
| `Bookings`, `BookingSeats` | ✅ `BookingDAO` | `createOnlineBooking`, **`applyPromotionToBooking`**, **`removePromotionFromBooking`**, **`cancelOnlinePendingBooking`**, staff OFFLINE |
...
## 6b. `PaymentServlet` — Thanh toán stub & voucher (FR-14 / FR-22 / FR-16)
...
UI: poster phim, ghế, breakdown (subtotal / giảm giá / VAT / tổng), countdown 10 phút, khối voucher (`.pay-promo-*`), nút VNPay/MoMo disabled (stub FR-16).
...
UI: input mã + breakdown giảm giá trên [`payment.jsp`](src/main/webapp/WEB-INF/views/customer/payment.jsp); JS uppercase khi gõ (`customer-payment.js`).

> **Lưu ý vận hành:** Admin list hiển thị **SCHEDULED** khi `start_date > now` dù `status = ACTIVE`. Mã tạo trên `/admin/promotions` cần **start_date ≤ hôm nay** mới áp được trên `/payment`.

### POST khác (stub FR-16)

Thông báo VNPay/MoMo chưa tích hợp — redirect lại payment với message.
...
| `.pay-*` | Trang payment — breakdown VAT, countdown, nút hủy | `Screen Design/Ticket booking/` |
...
- Countdown `#payCountdown` từ `data-expires-at` — hết giờ redirect checkout cùng suất
- Input `#payPromoCode`: tự uppercase khi gõ (đồng bộ với validate server `UPPER(code)`)
- Không poll API — trang reload sau apply/remove/cancel voucher
...
```
FR-11 ✅  →  FR-50 ✅  →  FR-12 ✅  →  FR-13 ✅  →  FR-14 ✅  →  FR-22 ✅  →  FR-16–18 (payment gateway)
                                                              ↓
...
1. **FR-16–18 chưa có** — trang `/payment` là stub thanh toán; VNPay/MoMo chưa implement (FR-22 voucher đã có).
2. **Chặn ghế 2 giai đoạn** — click → `SeatHolds` 10 phút; form POST → `Bookings` PENDING (xóa holds).
3. **Poll 2s hai chiều** — không realtime; race xử lý bằng validate DB + unique constraint.
4. **SQL Server booking ID** — `createOnlineBooking` dùng `OUTPUT INSERTED.id` (không `getGeneratedKeys` với `UNIQUEIDENTIFIER`).
5. **Pricing rules** — chỉ đọc ACTIVE; Manager chưa có UI CRUD (FR-49). Seed demo có sẵn trong `create_database.sql`.
6. **Voucher validate** — `validateForApply` kiểm tra ACTIVE + date range + usage limit; admin badge **SCHEDULED** ≠ áp được trên customer.
7. **Suất sau ngày thứ 7** không hiển thị — chỉ 7 tab cố định.
8. **`ShowtimesServlet` ở `controller`** (public); checkout/payment authenticated ở `controller.customer`.
9. **Không CSRF** — form POST checkout/payment cần cân nhắc token khi harden bảo mật.
