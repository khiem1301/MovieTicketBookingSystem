...
| Cấu hình VAT / giá vé khác | ❌ Placeholder (ghi chú trên UI) |
...
├── config-list.jsp     # Cấu hình loyalty (SystemConfig)
└── .gitkeep
```
...
- Card **Thuế VAT** — placeholder: *"Quản lý quy tắc VAT (VatRules) sẽ được bổ sung ở phase tiếp theo"*
...
| 5 | Không audit log | Hành động lock/reset/config không ghi log |
...
| 8 | Config chỉ loyalty | UI chưa quản lý VAT (`VatRules`) hay key SystemConfig khác |
...
|-----------|-------------|
| Cấu hình VAT | CRUD `VatRules` qua admin UI |
