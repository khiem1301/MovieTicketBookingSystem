...
<main class="admin-page">
  <div class="container">
      ...
      <c:choose>
        ...
        <c:otherwise>
          <p style="font-size:13px;color:var(--text-muted);margin:0 0 8px;">
            Chưa có quy tắc nào trong khoảng ngày hiện tại — hệ thống dùng mặc định <strong>8%</strong> khi đặt vé.
          </p>
        </c:otherwise>
      </c:choose>
    </div>
      ...
      <c:if test="${not empty scheduledList or not empty history}">
        <div class="admin-table-wrap">
          <table class="admin-table">
            ...
            <tbody>
              <c:forEach var="rule" items="${scheduledList}">
                <tr>
                  ...
                  <td>
                    <span class="admin-badge admin-badge--role">ĐÃ LÊN LỊCH</span>
                  </td>
                  <td class="cell-muted">
                    <fmt:formatDate value="${rule.createdAt}" pattern="dd/MM/yyyy HH:mm"/>
                  </td>
                  <td>
                    <a href="${pageContext.request.contextPath}/admin/vat?edit=${rule.id}#vat-edit-form"
                       class="admin-btn admin-btn--ghost" style="padding:6px 12px;font-size:12px;">
                      Sửa
                    </a>
                  </td>
                </tr>
        ...
        <c:if test="${not empty history}">
          <p class="admin-stats" style="margin-top:12px;">
            Lịch sử đã hết hiệu lực: <strong><c:out value="${historyTotal}"/></strong> quy tắc
          </p>
          <c:set var="pgCurrent" value="${currentPage}"/>
          <c:set var="pgTotal" value="${totalPages}"/>
          <c:set var="pgTotalItems" value="${historyTotal}"/>
          <%@ include file="/WEB-INF/views/admin/pagination.jspf" %>
        </c:if>
      </c:if>

      <c:if test="${empty scheduledList and empty history}">
    ...
    <c:if test="${not empty editRule}">
      <div class="admin-card" id="vat-edit-form">
        <h2 class="admin-section-title">Chỉnh sửa quy tắc đã chọn</h2>
        <p style="font-size:13px;color:var(--text-muted);margin-bottom:20px;">
          Đang sửa: <strong><c:out value="${editRule.ruleName}"/></strong>
          — áp dụng từ <fmt:formatDate value="${editRule.startDate}" pattern="dd/MM/yyyy"/>.
        </p>
        ...
        <form class="admin-form admin-form--wide" method="post"
          ...
          <div class="admin-field">
            <label class="admin-label" for="editRuleName">Tên quy tắc *</label>
            <input type="text" id="editRuleName" name="ruleName" class="admin-input" required
                   maxlength="100" value="<c:out value='${editRule.ruleName}'/>"/>
          </div>

          <div class="admin-field">
            <label class="admin-label" for="editVatRate">Thuế suất VAT (%) *</label>
            <input type="number" id="editVatRate" name="vatRate" class="admin-input" required
                   min="0" max="100" step="0.01"
                   value="<fmt:formatNumber value='${editRule.vatRate}' minFractionDigits='0' maxFractionDigits='2' groupingUsed='false'/>"/>
          </div>

          <div class="admin-field">
            <label class="admin-label" for="editStartDate">Ngày bắt đầu áp dụng *</label>
            <input type="date" id="editStartDate" name="startDate" class="admin-input" required
                   value="<c:out value='${editStartValue}'/>"/>
          </div>

          <div class="admin-form-actions">
            <button type="submit" class="admin-btn admin-btn--primary">Lưu thay đổi</button>
            <a href="${pageContext.request.contextPath}/admin/vat" class="admin-btn admin-btn--ghost">Đóng</a>
          </div>
        </form>

        <form method="post" action="${pageContext.request.contextPath}/admin/vat/cancel"
              style="margin-top:12px;"
              onsubmit="return confirm('Hủy quy tắc đã lên lịch này?');">
          <input type="hidden" name="ruleId" value="<c:out value='${editRule.id}'/>"/>
          <button type="submit" class="admin-btn admin-btn--ghost" style="color:#ff8a80 !important;">
            Hủy quy tắc này
          </button>
        </form>
      </div>
