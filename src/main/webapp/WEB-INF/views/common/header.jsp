...
<header class="site-header">
  <div class="header-inner">
    ...
    <form class="header-search" action="${pageContext.request.contextPath}/movies" method="get">
      ...
      <input type="text" name="q" placeholder="Tìm kiếm phim..." autocomplete="off"
      ...
      <div class="nav-item">
        ...
        <div class="dropdown-menu dropdown-genres">
          <c:choose>
            ...
            <c:otherwise>
              <a href="${pageContext.request.contextPath}/movies?genre=action">Hành động</a>
              <a href="${pageContext.request.contextPath}/movies?genre=romance">Tình cảm</a>
              <a href="${pageContext.request.contextPath}/movies?genre=horror">Kinh dị</a>
              <a href="${pageContext.request.contextPath}/movies?genre=scifi">Viễn tưởng</a>
              <a href="${pageContext.request.contextPath}/movies?genre=animation">Hoạt hình</a>
              <a href="${pageContext.request.contextPath}/movies?genre=comedy">Hài</a>
              <a href="${pageContext.request.contextPath}/movies?genre=drama">Chính kịch</a>
              <a href="${pageContext.request.contextPath}/movies?genre=thriller">Kịch tính</a>
            </c:otherwise>
          </c:choose>
        </div>
      </div>
      ...
      <c:choose>
        <c:when test="${not empty sessionScope.loggedUser}">
          <div class="user-menu nav-item">
            <div class="user-info">
              <c:choose>
                <c:when test="${not empty sessionScope.loggedUser.avatarUrl}">
                  <img class="user-avatar"
                       src="<c:out value='${sessionScope.loggedUser.avatarUrl}'/>"
                       alt="Avatar"/>
                </c:when>
                <c:otherwise>
              ...
              <span class="user-name"><c:out value="${sessionScope.loggedUser.fullName}"/></span>
              <span class="nav-arrow" style="font-size:10px;color:var(--text-muted)">▾</span>
            </div>
            <div class="dropdown-menu" style="right:0;left:auto;">
              <a href="${pageContext.request.contextPath}/profile">Tài khoản</a>
              <c:if test="${sessionScope.userRole == 'CUSTOMER'}">
                <a href="${pageContext.request.contextPath}/booking-history">Lịch sử đặt vé</a>
                <a href="${pageContext.request.contextPath}/loyalty">Điểm thưởng</a>
              </c:if>
